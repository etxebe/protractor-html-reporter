...
var HTMLReport = function() {
  ...
  var generateSummaries = function(reportXml) {
    ...
    for (var i=0; i<totalSuites; i++) {
      ...
      for (var j=0; j<totalCasesPerSuite; j++) {
        ...
        //get test cases screenshots names (on failure only)
          if (testCasesResults[j] != 'Failed') {
            screenshotsNamesOnFailure.push('None');
          } else {
            screenshotsNamesOnFailure.push('chrome-' + suite.name + ' ' + testCasesNames[j] + '.png');
          }
      }
...
this.from = function(reportXml, testConfig) {
    //generate statistics
    var testDetails = generateSummaries(reportXml);

    //set report data (name, time elapsed etc.)
    report.name = testConfig.reportTitle;
